...
from global_config.global_config import CROSS_VALIDATION_WHERE_TARGETS_PATH, CROSS_VALIDATION_WHERE_PREDICTIONS_PATH

sns.set_style("whitegrid")

GROUND_TRUTH_PATH = CROSS_VALIDATION_WHERE_TARGETS_PATH
PREDICTED_PATH = CROSS_VALIDATION_WHERE_PREDICTIONS_PATH
NUM_FOLDS = 5
APPROXIMATE = True
...
plt.xlim([0.0, 100])
plt.ylim([0.0, 105])
plt.title("WHERE: Precision-recall plot")
...
sns.despine()
plt.savefig("./figures/roc_where.png")
...
ax.get_legend().remove()
sns.despine()
plt.savefig("./figures/roc_where_zoomed.png")
