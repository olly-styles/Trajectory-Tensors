...
for fold_num in range(1, NUM_CROSS_VAL_FOLDS + 1):
    ...
    test_targets = np.load(os.path.join(CROSS_VALIDATION_WHEN_TARGETS_PATH, "test_fold" + fold + ".npy"))

    all_predictions = np.zeros((len(test_targets), NUM_CAMERAS, FUTURE_TRAJECTORY_LENGTH), dtype="float")
    for camera in range(1, NUM_CAMERAS + 1):
        ...
        if len(camera_targets) == 0:
            # Set all predictions to be the same
            mean_predictions = np.ones((NUM_CAMERAS, FUTURE_TRAJECTORY_LENGTH)) * (1 / FUTURE_TRAJECTORY_LENGTH)
        else:
    ...
    if not os.path.exists(CROSS_VALIDATION_WHEN_TRAINING_SET_MEAN_PATH):
        os.makedirs(CROSS_VALIDATION_WHEN_TRAINING_SET_MEAN_PATH)
    np.save(os.path.join(CROSS_VALIDATION_WHEN_TRAINING_SET_MEAN_PATH, "test_fold" + fold + ".npy"), all_predictions)
