...
for fold_num in range(1, NUM_CROSS_VAL_FOLDS + 1):
    ...
    all_predictions = np.zeros(
        (len(test_inputs), NUM_CAMERAS, FUTURE_TRAJECTORY_LENGTH) + BASE_HEATMAP_SIZE, dtype="bool"
    )

    for row in range(len(test_inputs)):
        print(row, "of", len(test_inputs))
        ...
        if len(cam_inputs) == 0:
            top_prediction = np.zeros((NUM_CAMERAS, FUTURE_TRAJECTORY_LENGTH) + BASE_HEATMAP_SIZE)
        else:
    ...
    np.save(
        os.path.join(CROSS_VALIDATION_WHERE_MOST_SIMILAR_TRAJECTORY_PATH, "test_fold" + fold + ".npy"), all_predictions
    )
