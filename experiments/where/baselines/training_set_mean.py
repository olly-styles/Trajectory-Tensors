...
for fold_num in range(1, NUM_CROSS_VAL_FOLDS + 1):
    ...
    test_targets = np.load(os.path.join(CROSS_VALIDATION_WHERE_TARGETS_PATH, "test_fold" + fold + ".npy"))

    all_predictions = np.zeros((len(test_targets), NUM_CAMERAS, FUTURE_TRAJECTORY_LENGTH) + BASE_HEATMAP_SIZE)
    ...
    for camera in range(1, NUM_CAMERAS + 1):
        ...
        if len(camera_targets) == 0:
            ...
            mean_predictions = np.ones((NUM_CAMERAS, FUTURE_TRAJECTORY_LENGTH) + BASE_HEATMAP_SIZE) * (
    ...
    np.save(os.path.join(CROSS_VALIDATION_WHERE_TRAINING_SET_MEAN_PATH, "test_fold" + fold + ".npy"), all_predictions)
