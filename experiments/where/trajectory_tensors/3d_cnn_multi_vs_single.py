...
for multi_view_tensors in [True, False]:
    ...
    if multi_view_tensors:
        MODEL_SAVE_PATH = os.path.join(CROSS_VALIDATION_MODELS_PATH_WHERE, "3dcnn")
        PREDICTION_SAVE_PATH = os.path.join(CROSS_VALIDATION_WHERE_PREDICTIONS_PATH, "trajectory_tensors", "3dcnn")
    else:
        MODEL_SAVE_PATH = os.path.join(CROSS_VALIDATION_MODELS_PATH_WHERE, "3dcnn_single_view")
