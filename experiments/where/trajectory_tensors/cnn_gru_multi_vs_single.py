...
for multi_view_tensors in [True, False]:
    ...
    if multi_view_tensors:
        MODEL_SAVE_PATH = os.path.join(CROSS_VALIDATION_MODELS_PATH_WHERE, "cnn_gru")
        PREDICTION_SAVE_PATH = os.path.join(CROSS_VALIDATION_WHERE_PREDICTIONS_PATH, "trajectory_tensors", "cnn_gru")
    else:
        MODEL_SAVE_PATH = os.path.join(CROSS_VALIDATION_MODELS_PATH_WHERE, "cnn_gru_single_view")
            ...
            if epoch % 3 == 0 and epoch != 0:
                ...
                if loss <= best_loss:
                    ...
                    if not MODEL_SAVE_PATH is None:
                        ...
                        torch.save(
                            best_temporal_decoder.state_dict(),
                            os.path.join(MODEL_SAVE_PATH, temporal_decoder_save_name),
                        )
                        torch.save(
                            best_spatial_decoder.state_dict(), os.path.join(MODEL_SAVE_PATH, spatial_decoder_save_name)
                        )
